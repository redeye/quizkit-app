...
This project is an experiment in creating a simple system based on machine readable content, created by a machine. Quiz's are easy fodder for experimenting with structured data as frankly there's not a lot to them and they're predictable. With the JSON data format created for a simple quiz, now we can get GPT (40) to search a website and create questions from it, outputting in JSON. I needed a simple framework to parse these JSON files into an "experience"; and here we are. A really simple framework for creating quiz's (quizzes?). 
